...
__global__ void kernBilateralFilter(
	float* denoised, float* luminance, float* normal, float* depth, float sigmaN, float sigmaD, float sigmaL);

class Denoiser {
public:
	Denoiser(Scene& Scene, PathTracer& PathTracer) :scene{ Scene }, pathTracer{ PathTracer } {}
	~Denoiser();
